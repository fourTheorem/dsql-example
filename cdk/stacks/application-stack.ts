...
export class ApplicationStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const dbEndpoint = ssm.StringParameter.fromStringParameterName(
      this,
      "DbEndpointParam",
      "/dsql-example/endpoint",
    ).stringValue;
    const dbClusterId = ssm.StringParameter.fromStringParameterName(
      this,
      "DbClusterId",
      "/dsql-example/cluster-id",
    ).stringValue;
    ...
    apiFunction.addToRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          `arn:${this.partition}:dsql:${this.region}:${this.account}:cluster/${dbClusterId}`,
        ],
      }),
    );
    ...
    for (const [path, methods] of Object.entries(pathMethods)) {
      const pathResource = parentResource.addResource(path, {
